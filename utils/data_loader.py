...
def load_data_and_vocab(opt, load_train=True):
    ...
    if load_train:  # load training dataset
        if not opt.one2many:  # load one2one dataset
            ...
            train_one2one_dataset = KeyphraseDataset(train_one2one, word2idx=word2idx, idx2word=idx2word, type='one2one', load_train=load_train)
            ...
            train_one2many_dataset = KeyphraseDataset(train_one2many, word2idx=word2idx, idx2word=idx2word, type='one2many', delimiter_type=opt.delimiter_type, load_train=load_train)
